<script>
	...
	const tableHistoryEndpoint = `${host}sales-activation/get-sales-activations`;
	const exportEndpoint = `${host}sales-activation/export-sales-activations`;
	const searchHistoryEndpoint = `${host}sales-activation/search`;
	const formHistoryEndpoint = `${host}sales-activation/sales-activations-form`;
	const summerizeEndpoint = $state(`${host}sales-activation/get-sales-activation/summary`);
	const filterEndpoint = `${host}sales-activation/all-filters`;

	const tableEndpoint = `${host}sales-activation/get-sales-activation-items`;
	const searchEndpoint = `${host}sales-activation/search-sales-activation-items`;
	const formEndpoint = `${host}sales-activation/sales-activation-items-form`;
	const table_id_data = `${host}sales-activation/get-sales-activation-item/`;

	let submitUrl = $state(`${host}sales-activation/create-sales-activation-item`);
	const updateUrl = `${host}sales-activation/update-sales-activation-item`;
	const deleteUrl = `${host}sales-activation/delete-sales-activation-item`;
	...
	let detailsTitle = $state('Activation Items');
	let detailsEndpointById = $state('');

	function handleMarkerClicked(body) {
		point_of_sale_id = body['id'];
		point_of_sale_name = body['name'];
		submitUrl = `${host}sales-activation/create-sales-activation-item/${point_of_sale_id}`;
		activeTab = 'Details';
	}
	...
	async function submitAll() {
		...
		if (point_of_sale_id != '') {
			let response = await fetch(
				`${host}sales-activation/create-sales-activation-link/${point_of_sale_id}`,
				{
					method: 'POST',
					headers: {
	...
	async function rowClicked(row) {
		...
		try {
			const data = await fetchTable(
				`${host}sales-activation/get-items-by-activation/${row.id}`,
				token
			);

<script>
	...
	$: load(span, timeline, territory, route, user_id, category, parentProduct, product);
	...
	async function fetchData(
		...
		try {
			let data = await fetchOrderSalesContribution(
				selectedSpan,
				selectedTimeline,
				selectedTerritory,
				selectedRoute,
			...
			const chartData = series.map((item) => ({
				period: item.label,
				sales: item.actual_conversions,
				target: item.target_conversions,
			...
			console.error('Error fetching order vs target data:', error);
		} finally {
		}
	}
	...
	async function load(
		...
		chart = Highcharts.chart('containerOR', {
			chart: {
				type: 'column',

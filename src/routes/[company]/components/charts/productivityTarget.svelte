<script>
	...
	import { fetchProductivityTarget } from '../../../../apis/api-utils.js';
	...
	async function fetchData(
		...
		try {
			let data = await fetchProductivityTarget(
				selectedSpan,
				selectedTimeline,
				selectedTerritory,
				selectedRoute,
			...
			const chartData = series.map((item) => ({
				period: item.label,
				sales: item.actual_productivity,
				target: item.target_productivity,
	...
	async function load(
		...
		chart = Highcharts.chart('containerPR', {
			chart: {
				type: 'column',

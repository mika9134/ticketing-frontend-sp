<script>
	...
	import { fetchPenetrationTarget } from '../../../../apis/api-utils.js';
	...
	async function fetchData(
		...
		try {
			let data = await fetchPenetrationTarget(
				selectedSpan,
				selectedTimeline,
				selectedTerritory,
				selectedRoute,
			...
			const chartData = series.map((item) => ({
				period: item.label,
				sales: item.actual_penetrations,
				target: item.target_penetrations,
	...
	async function load(
		...
		chart = Highcharts.chart('containerPN', {
			chart: {
				type: 'column',

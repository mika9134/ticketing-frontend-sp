<script>
	...
	{
	  if (expanded_row_id.includes(row['id'])) {expanded_row_id.splice(expanded_row_id.indexOf(row['id']), 1);} else {expanded_row_id.push(row['id']);  table_data.push(...row['hidden']);}
	}
	...
	{	  
	  ...
	  if (expanded_row_id.includes(row['id'])) {
	    ...
	    expanded_row_id.splice(expanded_row_id.indexOf(row['id']));
	...
	export async function loadTable() {
		try {
			...
			// Fetch table data
			table_data_total = await fetchTable(get_table_endpoint, getAccessToken());
				...
				<table	class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm text-left text-gray-700 dark:text-gray-300"	>
					...
					<tbody>
						{#each paginatedData as row, i}
							...
							<tr class="bg-gray-100 dark:bg-[#002a47] hover:bg-gray-50 dark:hover:bg-gray-800 transition duration-200">
							 {#if row['hidden'] && row['hidden'].length > 0}
								<td	class="px-4 py-4 font-medium bg-gray-200 dark:bg-[#002a47] text-gray-900 dark:text-white border-b border-primary-500 dark:border-gray-600"
								>
									<div class="flex items-center">
										<input
											id="checkbox-{row.id}"
							...
							{/if}
								{#each columns as col,i }
									...
									{#if row[col] === null}
									  {#if row['hidden'] && row['hidden'].length > 0}
										<td
											class="px-2 py-3 max-w-[12rem] truncate border-b bg-gray-200 dark:bg-[#002a47] border-primary-500 dark:border-gray-600 hover:underline dark:text-primary-400"
											>N/A</td
										>
										{:else}
																			<td
									...
									{:else if col === 'description' || col === "roles"|| col === "domain"  ||(table_name === "Scope Group" && col === "organizations")}
										<!-- Apply special styling to the description column -->
										 {#if row['hidden'] && row['hidden'].length > 0}
										<td class="px-2 py-4 max-w-[12rem] whitespace-pre-wrap bg-gray-200 dark:bg-[#002a47] break-words border-b border-primary-500 dark:border-gray-600 hover:underline dark:text-primary-400"
										>
											{#if col === "roles"}
	                     ...
	                     {/if}
									...
									{:else if col !== 'hidden'}
										...
										<td
											class={row['hidden'] && row['hidden'].length > 0 ? "px-2 py-4 max-w-[16rem] bg-gray-200 dark:bg-[#002a47] break-word border-b border-primary-500 dark:border-gray-600 hover:underline dark:text-primary-400" : "px-2 py-4 max-w-[16rem] break-word border-b border-primary-500 dark:border-gray-600 hover:underline dark:text-primary-400"}
											>
											
											<div class="flex gap-2">
											{#if i == 0 && row['hidden'] && row['hidden'].length > 0}
                      {#if (expanded_row_id.includes(row['id']))}
											<button class="color-green-500" onclick={() => {collapseTableItem(row)}}>


	   <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="m12 6.586-8.707 8.707 1.414 1.414L12 9.414l7.293 7.293 1.414-1.414L12 6.586z"/></svg>
											</button>
											{:else}
											
											<button class="color-green-500"  onclick={() => {expandTableItem(row)}}>
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="currentColor" d="M12 17.414 3.293 8.707l1.414-1.414L12 14.586l7.293-7.293 1.414 1.414L12 17.414z"/></svg>
                    ...
                    {/if}
								...
								<!-- Action section -->
								<td class={row['hidden'] && row['hidden'].length > 0 ? "px-6py-4 border-b bg-gray-200 dark:bg-[#002a47] border-primary-500 dark:border-gray-600" : "px-6py-4 border-b border-primary-500 dark:border-gray-600"}>
									<div
									...
									>
										...
										<div class="relative inline-block text-left">
											...
											<button
											type="button"
											class="inline-flex justify-center w-full rounded-md px-2 py-1 text-sm font-medium text-gray-700 dark:text-white hover:bg-gray-200 dark:bg-[#002a47] dark:hover:bg-gray-700 focus:outline-none"
											onclick={async() => {
												...
												if(["Organization"].includes(table_name)){
													...
													console.log('hidden',children_organization,children_organization.length)

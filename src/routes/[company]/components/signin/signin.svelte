<script lang="ts">
	...
	async function load() {
		...
		if (access_token) {
			let userRoles = await fetchRoles(
				`${host}role/my-role`,
				access_token,
			);
			...
			let contibutePermissionIndex =
				Object.values(userRoles).indexOf("contribute");
			let managePermissionIndex =
				Object.values(userRoles).indexOf("manage");
			...
			if (userRoles["Dashboard"] !== undefined) {
				...
				for (let x = 2; x < Object.keys(userRoles).length; x += 1) {
					if (
						...
						current_module =
							module_path[Object.keys(userRoles)[roleIndex]];
		...
		if (
			...
			const check_url = `${host_super}provider/admin`;
		...
		try {
			...
			if (access_token) {
				if (request_deletion_endpoint != null) {
					const request_message = await updateForm(
	...
	let slides = [
		{
			...
			content:
				"A Sales Force Automation App to enhance your sales productivity.",
		},
		{
		...
		},
		{
			...
			content:
				"Manage your POS system and track performance with real-time data to improve sales.",
		},
		{
...
<main>
	...
	<div class="min-h-screen flex flex-col bg-white">
		<header class="bg-[#f2f2f2] px-10 tracking-wide">
			<div class="flex gap-8">
				<div class="lg:flex lg:items-center">
					<img
						class="w-40 mb-2 border-none"
						src="/images/BlueSpark.png"
						alt="BlueSpark"
					/>
				</div>
			</div>
		...
		<div
			class="flex-grow lg:grid grid-cols-2 bg-white h-full overflow-auto self-center w-full"
		>
					...
					<div class="p-10">
						{#each slides as slide, index (index)}
							{#if activeSlide === index + 1}
								<div
									class="p-2 text-[#1a1a1a] text-center rounded-lg"
								>
									<img
										src={slide.icon}
										alt="slide Icons"
										class="slide-icon mx-auto"
									/>
									<div class="w-full lg:w-4/5 mx-auto">
										<h1
											class="mt-10 text-3xl lg:text-4xl font-bold text-[#1c467e]"
										>
											{slide.title}
										</h1>
									</div>
						...
						<div class="px-6">
							{#each slides as slide, index (index)}
								<button
									...
									class:bg-[#2a4c6b]={activeSlide ===
										index + 1}
									class:bg-[#c8d9e6]={activeSlide !==
										index + 1}
			...
			<div class="flex justify-center self-center text-[#1a1a1a] m-2">
				<div
					class="bg-[#1c467e] shadow-2xl w-full max-w-md rounded-xl mx-auto p-8"
				>
					<h2
					...
					>
						Sign In
						<img
							src={!isSuperAdmin && $companyStore
								? $companyStore.logo
								: "/images/BlueSpark2.png"}
							alt=""
							class="w-32"
						/>
					</h2>
					...
					<form onsubmit={handleLogin} class="text-sm lg:text-base">
						<div class="mb-8">
							<label for="username" class="block text-[#ffffff]"
								>Username</label
							>
							<input
								id="username"
						...
						<div class="mb-6 relative">
							...
							<div class="mt-4">
								<label
									...
									class="font-medium text-gray-100 hover:text-blue-400"

<script>
  import {deleteItem} from '../../apis/form.js';
  ...
  export let delete_url;
  export let onCloseModal;
  export let onDeleteItem;
  export let token;
  export let deletePopupMessage;

  function handleDelete() {
    if(Array.isArray(selectedRow))
    {
    selectedRow.forEach((t) => {
    onDeleteItem(t); // Call the delete function with the row ID
    });
    }
    else {
    onDeleteItem(selectedRow.id);
    }

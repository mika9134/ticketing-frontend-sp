<script>
    ...
    import { fetchSalesVsTarget } from "../../apis/api-utils.js"; // Adjust the path as needed
    ...
    let salesVsTargetData = []; // To store the fetched data
    let labels = []; // Dates (periods)
    let targetSales = []; // Target values
    let actualSales = []; // Actual sales achieved
    let barChart;
  
    // Fetch data from the API
    async function fetchData() {
      try {
        salesVsTargetData = await fetchSalesVsTarget(timeline, span);
  
        // Process the fetched data
        labels = salesVsTargetData.map((item) => item.period); // Extract periods (dates)
        targetSales = salesVsTargetData.map((item) => item.target); // Extract target values
        actualSales = salesVsTargetData.map((item) => item.sales); // Extract actual sales
    ...
    function updateChart() {
      if (barChart) {
        ...
        barChart.data.datasets[0].data = targetSales;
        barChart.data.datasets[1].data = actualSales;
        barChart.update();
      }
    }
          ...
          datasets: [
            {
              ...
              backgroundColor: "rgba(200, 200, 200, 0.5)", // Light grey for target
              borderWidth: 0,
            },
            {
              label: "Actual Sales",
              data: [], // Placeholder data
              backgroundColor: "#4CAF50", // Green for actual sales
            },
          ],
        },
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: "Sales vs Target (Daily Performance)",
            },
          },
        },
      });
  ...
  <canvas bind:this={chartCanvas} width="800" height="350"></canvas>
...
<li>
  ...
  <script>
      ...
      if (typeof window !== "undefined") {
        ...
        barChart = new Chart(ctx, {
          ...
          options: {
            responsive: true,
            plugins: {
              title: { display: true, text: "Sales vs Target", color:"white" },
              legend: {
                labels: {
                  usePointStyle: true,
                  pointStyle: "circle",
    ...
    async function fetchData(selectedSpan, selectedTimeline) {
      ...
      try {
        const data = await fetchSalesVsTarget(selectedSpan, selectedTimeline);
        if (!data || data.length === 0) {
          console.error("No data available to display the chart.");
          return;
        }
  
        const labels = data.map((item) => item.period); // Extract periods for x-axis
        const targetSales = data.map((item) => item.target); // Extract target values
        const actualSales = data.map((item) => item.sales); // Extract sales values
        const salesPercentage = data.map((item) => item.salesPercentage); // Extract sales percentage
  
        // Limit the number of bars to `maxBars`
        const limitedLabels = labels.slice(0, maxBars);
        const limitedTargetSales = targetSales.slice(0, maxBars);
        const limitedActualSales = actualSales.slice(0, maxBars);
  
        updateChart(limitedLabels, limitedTargetSales, limitedActualSales, salesPercentage);
      } catch (error) {
        console.error("Error fetching sales vs target data:", error);
      } finally {
